...
class Transition:
    def __init__(self, _from, _to, symbol, stack, add, emit):
        self._from = _from
        self._to = _to
        self.symbol = symbol
        self.stack = stack
...
class NPDA:
    ...
    def check(self, word):
        ...
        emited = ""
        self.advance([self.initial_stack], deepcopy(word), 0, deepcopy(emited))
        print()

    def advance(self, stack, word, level=0, emited=""):
        if self.current_node.is_final:
            word_parsed = self.word[:len(self.word) - len(word)]
            if word_parsed == self.word:
                # if word_parsed not in self.found:
                self.found += [word_parsed]
                # print(f"{word_parsed} acceptat de npda")
                print(f"{emited}")
        ...
        for t in self.current_node.transitions:
            ...
            if (t.symbol == self.lam or (len(word) and t.symbol == word[0])) and t.stack == stack[-1]:
                ...
                new_emited = deepcopy(emited)
                if t.emit != self.lam:
                    new_emited += t.emit
                self.advance(new_stack, new_word, level+1, new_emited)
...
def parse_transition(f):
    ...
    for i in range(_count):
        line = f.readline().split()
        symbol, stack, add, emit = line[0], line[1], line[2], line[3]
        nodes[_from].add_transition(Transition(nodes[_from], nodes[_to], symbol, stack, add, emit))
